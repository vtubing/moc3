...
pub struct ArtMeshDrawableFlags(pub u8);

impl TryFrom<u8> for ArtMeshDrawableFlags {
  type Error = Error;
...
impl<T> Reading<ArtMeshDrawableFlags> for T
where
  T: Reading<u8>,
{
  fn read_one(&mut self) -> Result<ArtMeshDrawableFlags> {
    self.read_one()?.try_into()
  }
}

impl std::fmt::Debug for ArtMeshDrawableFlags {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    f.write_fmt(format_args!("DrawableFlags({:#010b})", self.0))
  }
}
